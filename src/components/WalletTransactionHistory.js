                    ...
                    <View style={styles.containsView}>
                        <View style={styles.statusStyle}>
                            ...
                            {item.type  && item.type == 'Credit'?
                                <Text style={styles.historyamounttextStyle}>{t('credited') + ' ' + settings.symbol + parseFloat(item.amount).toFixed(2)}</Text>
                            :null}
                            {item.type && item.type == 'Debit'?
                                <Text style={styles.historyamounttextStyle}>{t('debited') + ' ' + settings.symbol + parseFloat(item.amount).toFixed(2)}</Text>
                            :null}
                            {item.type && item.type == 'Withdraw'?
                                <Text style={styles.historyamounttextStyle}>{t('withdrawn') + ' ' + settings.symbol + parseFloat(item.amount).toFixed(2)}</Text>
                            :null}   
                            <Text style={styles.textStyle}>{t('transaction_id')} {item.txRef.startsWith('wallet')?item.transaction_id:item.txRef}</Text>
